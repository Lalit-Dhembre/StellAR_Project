...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT)) {
      if (!isCameraActive) { DisableCursor(); isCameraActive = true; }
      ...
      if (IsKeyDown(KEY_W)) { isTracking = false;
      ...
      if (IsKeyDown(KEY_S)) { isTracking = false;
      ...
      if (IsKeyDown(KEY_D)) { isTracking = false;
      ...
      if (IsKeyDown(KEY_A)) { isTracking = false;
      ...
      if (isCameraActive) { EnableCursor(); isCameraActive = false; }
    }
    ...
    // ── Mouse Picking ────────────────────────────────────────────────
    {
      ...
      Rectangle pickGuardRight = {(float)screenWidth - 516.0f, 10.0f, 506.0f, 146.0f};
    ...
    for (size_t f = 0; f < activeFragments.size(); f++) {
      ...
      if (currentState == PLAYING) {
        ...
        if (activeFragments[f].life <= 0.0f) { activeFragments[f].isAlive = false; continue; }
      }
    ...
    DrawControlsPanel(screenWidth, screenHeight, selectedPlanet, isTracking,
                      currentState, activePlanets, initialPlanets, activeFragments,
                      prevSelectedPlanet, prevSliderMass, prevSliderRadius,
                      settledMass, toastTimer);
    DrawStatusPanel(screenWidth, currentState, cameraSpeed);

    ProcessToastAndNarration(screenWidth, screenHeight, selectedPlanet,
                             prevSelectedPlanet, activePlanets,
                             prevSliderMass, prevSliderRadius, settledMass,
                             toastTimer, toastText, sizeof(toastText), dt);
