...
void ProcessMobileInput(int screenWidth, int screenHeight, Camera3D &camera,
                        MobileInputState &input, float effectiveCameraSpeed,
                        bool &isTracking, bool isCameraActive, float dt) {
  if (isCameraActive) {
    return;
  }

  const float jsBaseRadius = 156.0f;
  const float jsThumbRadius = 54.0f;
  const float jsDeadZone = 0.10f;
  const float margin = 96.0f;

  Vector2 jsBase = {margin + jsBaseRadius,
                    (float)screenHeight - margin - jsBaseRadius};

  Rectangle btnUp = {(float)screenWidth - 192.0f, (float)screenHeight - 432.0f,
                     144.0f, 120.0f};
  ...
  if (isRectPressed(btnUp)) {
    isTracking = false;
    camera.position =
        Vector3Add(camera.position, Vector3Scale(worldUp, moveAmount));
    camera.target = Vector3Add(camera.target, Vector3Scale(worldUp, moveAmount));
  }
  if (isRectPressed(btnDown)) {
    isTracking = false;
    camera.position =
        Vector3Subtract(camera.position, Vector3Scale(worldUp, moveAmount));
    camera.target =
        Vector3Subtract(camera.target, Vector3Scale(worldUp, moveAmount));
  }
  ...
  if (tc > 0) {
    for (int t = 0; t < tc; t++) {
      ...
      if (input.joystickActive && tid == input.joystickTouchPointId) {
        ...
        if (dist > jsBaseRadius) {
          dx = (dx / dist) * jsBaseRadius;
          dy = (dy / dist) * jsBaseRadius;
        }
        input.joystickThumb = {input.joystickCenter.x + dx,
                               input.joystickCenter.y + dy};
        ...
        camForward =
            Vector3RotateByAxisAngle(camForward, worldUp, -delta.x * 0.004f);
        camRight = Vector3Normalize(Vector3CrossProduct(camForward, worldUp));
        Vector3 newForward =
            Vector3RotateByAxisAngle(camForward, camRight, -delta.y * 0.004f);
  ...
  if (input.joystickActive) {
    float normX =
        (input.joystickThumb.x - input.joystickCenter.x) / jsBaseRadius;
    float normY =
        (input.joystickThumb.y - input.joystickCenter.y) / jsBaseRadius;
    if (fabsf(normX) > jsDeadZone) {
      isTracking = false;
      camera.position =
          Vector3Add(camera.position, Vector3Scale(camRight, normX * moveAmount));
      camera.target =
          Vector3Add(camera.target, Vector3Scale(camRight, normX * moveAmount));
    }
    if (fabsf(normY) > jsDeadZone) {
      isTracking = false;
  ...
  DrawCircleV(jsBase, jsBaseRadius, {50, 50, 50, 120});
  DrawCircleLinesV(jsBase, jsBaseRadius, {255, 255, 255, 160});
  DrawCircleV(input.joystickActive ? input.joystickThumb : jsBase, jsThumbRadius,
              input.joystickActive ? Color{255, 255, 255, 220}
                                   : Color{160, 160, 160, 170});
  DrawText("MOVE", (int)(jsBase.x - 36), (int)(jsBase.y - jsBaseRadius - 42), 29,
           LIGHTGRAY);
