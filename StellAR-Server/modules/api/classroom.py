...
def get_classroom(classroom_id):
    try:
        classroom_data = appwrite_service.query_records("classroom", select="*", filters={"id": classroom_id})
        
        if not classroom_data:
            return jsonify({'error': 'Classroom not found'}), 200
        
        classroom_author = appwrite_service.query_records("users", select="user_name", filters={"id": classroom_data[0]['created_by']})
        classroom_details = {
...
def create_classroom():
    try:
        ...
        response = appwrite_service.insert_record(
            "classroom",
            payload
        )
...
def join_classroom():
    try:
        ...
        classroom_id = appwrite_service.query_records("classroom", select="id", filters={"join_code": payload['join_code']})
        if not classroom_id:
...
def get_classroom_members(classroom_id):
    try:
        classroom_members = appwrite_service.query_records("classroom_members", select="*", filters={"classroom_id": classroom_id})
        users = []
        for i in classroom_members:
            user_data = appwrite_service.query_records("users", select="*", filters={"id": i['user_id']})
...
def get_classroom_joined(user_id):
    try:
        ...
        classroom_joined = appwrite_service.query_records("classroom_members", select="*", filters={"user_id": user_id})
        ...
        for i in classroom_joined:
            ...
            classroom_res = appwrite_service.query_records("classroom", select="name", filters={"id": i['classroom_id']})
            
            # Note: Fetching user details using i['user_id'] gets the member's name.
            user_res = appwrite_service.query_records("users", select="user_name", filters={"id": i['user_id']})

            # Fetch members to count them.
            members_res = appwrite_service.query_records("classroom_members", select="id", filters={"classroom_id": i['classroom_id']})
        ...
        print(f"Error in get_classroom_joined: {e}")
        return jsonify({'error': str(e)}), 500

@classroom_api.route('/api/classroommodels/<classroom_id>', methods=['GET'])
def get_classroom_models(classroom_id):
    try:
        classroom_models = appwrite_service.query_records("models", select="*", filters={"uploader_id": classroom_id})
